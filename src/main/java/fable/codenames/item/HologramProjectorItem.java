...
public class HologramProjectorItem extends Item {
    ...
    public HologramProjectorItem(Settings settings) {
        super(settings);
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        EntityType<?> type = ModMiscEntityTypes.HOLOGRAM_PROJECTOR.getEntityType();
        HologramProjectorEntity entity = (HologramProjectorEntity) type.create(serverWorld);

        if (entity == null) {
            return ActionResult.FAIL;
        }

        Direction face = getFace(context.getStack(), context.getPlayer() == null ? Direction.SOUTH : context.getPlayer().getHorizontalFacing());
        entity.refreshPositionAndAngles(spawnPos.x, spawnPos.y, spawnPos.z, face.asRotation(), 0.0f);
